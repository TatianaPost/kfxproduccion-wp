...
function ale_enqueue_styles() {
    ...
    wp_register_style( '1', THEME_URL . '/css/preview/1.css', array(), ALETHEME_THEME_VERSION, 'all');
    wp_register_style( '2', THEME_URL . '/css/preview/2.css', array(), ALETHEME_THEME_VERSION, 'all');
    wp_register_style( '3', THEME_URL . '/css/preview/3.css', array(), ALETHEME_THEME_VERSION, 'all');
    wp_register_style( '4', THEME_URL . '/css/preview/4.css', array(), ALETHEME_THEME_VERSION, 'all');
    wp_register_style( '5', THEME_URL . '/css/preview/5.css', array(), ALETHEME_THEME_VERSION, 'all');
    wp_register_style( '6', THEME_URL . '/css/preview/6.css', array(), ALETHEME_THEME_VERSION, 'all');
    wp_register_style( '7', THEME_URL . '/css/preview/7.css', array(), ALETHEME_THEME_VERSION, 'all');
    wp_register_style( '8', THEME_URL . '/css/preview/8.css', array(), ALETHEME_THEME_VERSION, 'all');
    wp_register_style( '9', THEME_URL . '/css/preview/9.css', array(), ALETHEME_THEME_VERSION, 'all');
    wp_register_style( '10', THEME_URL . '/css/preview/10.css', array(), ALETHEME_THEME_VERSION, 'all');

    wp_enqueue_style('aletheme_general_css');

    //Check if User have styles cookie, if no show the color from Theme Options.
    if(!isset($_COOKIE["bonitacolor"])) {
        wp_enqueue_style(ale_get_option('color_style'));
    } else {
        wp_enqueue_style($_COOKIE["bonitacolor"]);
    }
...
function ale_enqueue_scripts() {
    ...
    if(get_post_type() == "gallery"){
        wp_enqueue_script( 'jquery.fullscreen-min' );
    }
    if(is_blog() or is_page_template('template-blog-two.php') or is_page_template('template-blog-three.php') or is_page_template('template-gallery-filter.php') or is_page_template('template-gallery-three.php') or is_page_template('template-gallery-four.php')){
        wp_enqueue_script( 'jquery.isotope.min' );
    }
    if(is_page_template('template-home-two.php')){
...
<<?php echo $tag ?> <?php comment_class(empty( $args['has_children'] ) ? '' : 'parent') ?> id="comment-<?php comment_ID() ?>">
    ...
    <div class="commentbody">
        <div class="comment-author vcard">
            <span class="login"><?php printf(__('<cite class="fn">%s</cite>'), get_comment_author_link()) ?></span>
            <span class="timecomment"><?php echo get_comment_time(); ?></span>
            <span class="datecomment"><?php echo get_comment_date(); ?></span>
        <span class="date">
